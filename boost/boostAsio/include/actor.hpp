...
public:
  ...
  friend void send(Message msg, actor_address from, actor_address to) {
    ...
    post(to->executor_, [=, msg = std::move(msg)]() mutable {
...
protected:
  ...
  actor(any_io_executor e) : executor_(std::move(e)) {}
  ...
  void deregister_handler(void (Actor::*mf)(Message, actor_address)) {
    ...
    for (auto iter = handlers_.begin(); iter != handlers_.end(); ++iter) {
      if ((*iter)->message_id() == id) {
        auto mh =
            static_cast<mf_message_handler<Actor, Message> *>(iter->get());
  ...
  template <class Message> void tail_send(Message msg, actor_address to) {
    // Execute the message handler in the context of the target's executor.
    defer(to->executor_, [=, msg = std::move(msg), from = this] {
      to->call_handler(std::move(msg), from);
    });
  }
...
public:
  ...
  Message wait() {
    ...
    std::cout << (int)message_queue_.front() << std::endl;
...
public:
  explicit member(any_io_executor e) : actor(std::move(e)) {
...
int actor() {
  ...
  // Initialise the actors by passing each one the address of the next actor in
  // the ring.
...
void actor1() {
  ...
  single_thread_pool pools;
  post(pools.get_executor(), [] {
  ...
  std::thread t([&]() {
    int i = 0;
    while (i < 10) {
      defer(pools.get_executor(), [] { std::cout << "defer" << std::endl; });
      i++;
    }
  });
  std::thread t1([&]() {
    int i = 0;
    while (i < 10) {
      post(pools.get_executor(), [] { std::cout << "post" << std::endl; });
      i++;
    }
  });
  std::cout << "1" << std::endl;
