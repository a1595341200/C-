...
class DisPatch {
public:
  template <typename T> void update(int a, const T &b) {
    cout << a << "  " << b << endl;
  }
...
class RilService {
public:
  template <typename T> void notify(int type, const T &msg) {
...
public:
  ...
  template <typename T> void update(int type, const T &msg) {
    cout << type << "  " << msg << endl;
  }
  shared_ptr<RilCallBack> mRilCallBack;
