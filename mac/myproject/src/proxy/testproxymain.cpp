...
void TestProxyImpl::on_test_variant_finished(const Glib::RefPtr<Gio::AsyncResult> result,
...
void TestProxyImpl::on_test_byte_string_array_finished(const Glib::RefPtr<Gio::AsyncResult> result,
...
void TestProxyImpl::on_test_dict_struct_array_finished(const Glib::RefPtr<Gio::AsyncResult> result,
													   std::vector<std::tuple<Glib::ustring,
																			  std::map<Glib::ustring,
																					   Glib::VariantBase>>> expected) {
...
void TestProxyImpl::proxy_created(const Glib::RefPtr<Gio::AsyncResult> result) {
	...
	m_proxy->TestStringVariantDict(variantMapValue,
								   sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_string_variant_dict_finished),
											  variantMapValue));
	m_pending_calls++;

	/* Dictionary string -> string */
	m_proxy->TestStringStringDict(stringMapValue,
								  sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_string_string_dict_finished),
											 stringMapValue));
	m_pending_calls++;

	/* Dictionary uint -> int */
	m_proxy->TestUintIntDict(intMapValue,
							 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_uint_int_dict_finished),
										intMapValue));
	m_pending_calls++;

	/* Variant */
	m_proxy->TestVariant(variantValue,
						 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_variant_finished), variantValue));
	m_pending_calls++;

	/* Variant2 */
	m_proxy->TestVariant2(stringValue, variantValue,
						  sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_variant2_finished),
									 stringValue,
									 variantValue));
	m_pending_calls++;

	/* Byte string array */
	m_proxy->TestByteStringArray(inputStrVec,
								 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_byte_string_array_finished),
											inputStrVec));
	m_pending_calls++;

	/* Object path array */
	m_proxy->TestObjectPathArray(inputObjPathVec,
								 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_object_path_array_finished),
											inputObjPathVec));
	m_pending_calls++;

	/* String array */
	m_proxy->TestStringArray(stringArray,
							 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_string_array_finished),
										stringArray));
	m_pending_calls++;
	...
	/* Struct array */
	m_proxy->TestStructArray(structArray,
							 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_struct_array_finished),
										structArray));
	m_pending_calls++;

	/* Dict Struct array */
	m_proxy->TestDictStructArray(dictStructArray,
								 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_dict_struct_array_finished),
											dictStructArray));
	m_pending_calls++;

	/* Byte string */
	m_proxy->TestByteString(bytestring,
							sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_byte_string_finished), bytestring));
	m_pending_calls++;

	/* Signature */
	m_proxy->TestSignature(signatureValue,
						   sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_signature_finished), signatureValue));
	m_pending_calls++;

	/* Object path */
	m_proxy->TestObjectPath(objectPath,
							sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_object_path_finished), objectPath));
	m_pending_calls++;

	/* String */
	m_proxy->TestString(stringValue,
						sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_string_finished), stringValue));
	m_pending_calls++;

	/* Double */
	m_proxy->TestDouble(doubleValue,
						sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_double_finished), doubleValue));
	m_pending_calls++;

	/* UInt64 */
	m_proxy->TestUInt64(uint64Value,
						sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_uint64_finished), uint64Value));
	m_pending_calls++;
	...
	/* UInt16 */
	m_proxy->TestUInt16(uint16Value,
						sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_uint16_finished), uint16Value));
	m_pending_calls++;
	...
	/* Boolean */
	m_proxy->TestBoolean(booleanValue,
						 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_boolean_finished), booleanValue));
	m_pending_calls++;
...
//
	...
	m_proxy->TestTriggerInternalPropertyChange(42,
											   sigc::bind(sigc::mem_fun(this,
																		&TestProxyImpl::on_test_trigger_internal_property_change_finished),
														  42));
	m_pending_calls++;
	...
	printStatus("Property (read): TestPropReadByteStringArray",
				m_proxy->TestPropReadByteStringArray_get() == PropReadByteStringArrayValue);
	...
	printStatus("Property (read): TestPropReadObjectPathArrayValue",
				m_proxy->TestPropReadObjectPathArray_get() == PropReadObjectPathArrayValue);
	...
	printStatus("Property (read): TestPropReadStringArray",
				m_proxy->TestPropReadStringArray_get() == PropReadStringArrayValue);

	//std::string res = m_proxy->TestPropReadByteString_get();
	//res.push_back('\0');
	//std::cout << res << std::endl;
	printStatus("Property (read): TestPropReadByteString",
				m_proxy->TestPropReadByteString_get() == std::string("Value7"));
	...
	printStatus("Property (read): TestPropReadWriteByteStringArray",
				m_proxy->TestPropReadWriteByteStringArray_get() == PropReadWriteByteStringArrayValue);
	...
	printStatus("Property (read): TestPropReadWriteObjectPathArray",
				m_proxy->TestPropReadWriteObjectPathArray_get() == PropReadWriteObjectPathArrayValue);
	...
	printStatus("Property (read): TestPropReadWriteStringArrayValue",
				m_proxy->TestPropReadWriteStringArray_get() == PropReadWriteStringArrayValue);

	printStatus("Property (read): TestPropReadWriteByteString",
				m_proxy->TestPropReadWriteByteString_get() == "Value25");
	...
	printStatus("Property (read): TestPropReadWriteObjectPath",
				m_proxy->TestPropReadWriteObjectPath_get() == "/Value27");
	...
	m_proxy->TestPropReadWriteSignature_set("b",
											sigc::bind(sigc::mem_fun(this,
																	 &TestProxyImpl::on_test_prop_read_write_signature),
													   "b"));
	m_proxy->TestPropReadWriteObjectPath_set("/some/where",
											 sigc::bind(sigc::mem_fun(this,
																	  &TestProxyImpl::on_test_prop_read_write_object_path),
														"/some/where"));
	m_proxy->TestPropReadWriteString_set("Hello world",
										 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_string),
													"Hello world"));
	m_proxy->TestPropReadWriteDouble_set(7331,
										 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_double),
													7331));
	m_proxy->TestPropReadWriteUInt64_set(7332,
										 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_uint64),
													7332));
	m_proxy->TestPropReadWriteInt64_set(7333,
										sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_int64),
												   7333));
	m_proxy->TestPropReadWriteUInt_set(7334,
									   sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_uint),
												  7334));
	m_proxy->TestPropReadWriteInt_set(7335,
									  sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_int),
												 7335));
	m_proxy->TestPropReadWriteUInt16_set(7336,
										 sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_uint16),
													7336));
	m_proxy->TestPropReadWriteInt16_set(7337,
										sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_int16),
												   7337));
	m_proxy->TestPropReadWriteChar_set('X',
									   sigc::bind(sigc::mem_fun(this, &TestProxyImpl::on_test_prop_read_write_char),
												  'X'));
	m_proxy->TestPropReadWriteBoolean_set(true,
										  sigc::bind(sigc::mem_fun(this,
																   &TestProxyImpl::on_test_prop_read_write_boolean),
													 true));
