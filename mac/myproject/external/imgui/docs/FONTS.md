...
io.Fonts->AddFontFromFileTTF("C:\\Window\\Fonts\\seguiemj.ttf", 16.0f, &cfg, ranges);
```
...
- You may find a precompiled version binary_to_compressed_c.exe for Window inside the demo binaries package (see [README](https://github.com/ocornut/imgui/blob/master/docs/README.md)).
...
io.Fonts->AddFontFromFileTTF("../MyImage01.jpg", ...);
```
- In C/C++ and most programming languages if you want to use a backslash `\` within a string literal, you need to write it double backslash `\\`. At it happens, Window uses backslashes as a path separator, so be mindful.
```cpp
...
```
In some situations, you may also use `/` path separator under Window.
...
Or use Arial Unicode or other Unicode fonts provided with Window for full characters coverage (not sure of their licensing).
