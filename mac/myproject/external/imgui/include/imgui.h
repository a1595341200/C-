...
// Define attributes of all API symbols declarations (e.g. for DLL under Window)
...
enum ImGuiKey : int
{
    ...
    ImGuiMod_Super                  = 1 << 15, // Cmd/Super/Window
...
struct ImGuiIO
{
    ...
    // (default to use native Win32 clipboard on Window, otherwise uses a private clipboard. Override to access OS clipboard on other architectures)
    const char* (*GetClipboardTextFn)(void* user_data);
    void        (*SetClipboardTextFn)(void* user_data, const char* text);
    void*       ClipboardUserData;

    // Optional: Notify OS Input Method Editor of the screen position of your cursor for text input position (e.g. when using Japanese/Chinese IME on Window)
    // (default to use native imm32 api on Window)
    ...
    bool        KeySuper;                           // Keyboard modifier down: Cmd/Super/Window
...
//   - Window are generally trying to stay within the Work Area of their host viewport.
