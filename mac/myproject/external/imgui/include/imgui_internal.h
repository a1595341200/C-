...
#ifdef _WIN32
#define IM_NEWLINE                      "\r\n"   // Play it nice with Window users (Update: since 2018-05, Notepad finally appears to support Unix-style carriage returns!)
#else
#define IM_NEWLINE                      "\n"
#endif
...
// Window data saved in imgui.ini file
...
struct ImGuiContext
{
    ...
    // Window state
    ImVector<ImGuiWindow*>  Windows;                            // Window, sorted in display order, back to front
...
struct IMGUI_API ImGuiWindow
{
    ...
    // The best way to understand what those rectangles are is to use the 'Metrics->Tools->Show Window Rectangles' viewer.
    ...
    ImGuiWindow*            NavLastChildNavWindow;              // When going to the menu bar, we remember the child window we came from. (This could probably be made implicit if we kept g.Window sorted by last focused including child window.)
...
namespace ImGui
{
    ...
    // Window: Display Order and Focus Order
