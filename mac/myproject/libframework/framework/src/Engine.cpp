...
bool Engine::delState(std::shared_ptr<IState> state) {
    ...
    if (!sinfo || sinfo->active) {
        if (DBUG) LOG() << "del state fail" << printState(state);
        return false;
    }
    bool isParent = false;
    ...
    if (isParent) {
        if (DBUG) LOG() << "del state fail check is parent " << printState(state);
        return false;
    }
    mStateMap.erase(state);
    delete sinfo;
    if (DBUG) LOG() << "del state " << printState(state);
    return true;
}

void Engine::setInitialState(std::shared_ptr<IState> state) {
    mInitialState = state;
    if (DBUG) LOG() << "set initialState " << printState(state);
}

void Engine::transitionState(std::shared_ptr<IState> state) {
    mTransitState = state;
    if (DBUG) LOG() << "set transitionState " << printState(state);
}

void Engine::start() {
    if (DBUG) LOG() << "start engine " << printState(mInitialState);
    mHaltState->setEngine(this);
    StateInfo *sinfo = mStateMap[mInitialState];

    if (!mInitialState) {
        if (DBUG) LOG() << "unset initialState";
        return;
    }
...
void Engine::handleMessage(const std::shared_ptr<Message> &message) {
    if (DBUG) LOG() << "handleMessage " << message->what;
    if (!mQuit) {
...
void Engine::onHandleMessage(std::shared_ptr<Message> msg) {
    //如果init过 或者收到了Quit命令
    if (DBUG) LOG() << "onHandleMessage " << msg->what;
    if (mInit || msg->what == ENGINE_QUIT_CMD) {
        ...
        if (DBUG) LOG() << "onHandleMessage skip";
    }
}
...
void Engine::onHandleTransition(std::shared_ptr<Message> msg) {
    ...
    if (newState) {
        if (DBUG) LOG() << newState->toString();
    } else {
        if (DBUG) LOG() << " newState is nullptr ";
    }
    if (newState) {
        while (true) {
            if (DBUG) LOG() << "true";
...
void Engine::fillStateStack(StateInfo *info, bool checkActive) {
    if (DBUG) LOG() << "fillStateStack";
    do {
        mStateStack.push(info);
        if (DBUG) LOG() << "mStateStack push " << info->cState->toString();
        info = info->pStateInfo;
    } while (info && (!checkActive || !info->active));
}

void Engine::fillStateVector() {
    if (DBUG) LOG() << "fillStateVector";
    while (!mStateStack.empty()) {
        mStateVec.push_back(mStateStack.top());
        if (DBUG) LOG() << "mStateVec push " << mStateStack.top()->cState->toString();
        if (DBUG) LOG() << "mStateStack pop " << mStateStack.top()->cState->toString();
        mStateStack.pop();
    }
}
