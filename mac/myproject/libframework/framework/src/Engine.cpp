...
bool Engine::delState(std::shared_ptr<IState> state) {
	...
	if (!sinfo || sinfo->active) {
		LOG() << "del state fail" << printState(state);
		return false;
	}
	bool isParent = false;
	...
	if (isParent) {
		LOG() << "del state fail check is parent " << printState(state);
		return false;
	}
	mStateMap.erase(state);
	delete sinfo;
	LOG() << "del state " << printState(state);
	return true;
}

void Engine::setInitialState(std::shared_ptr<IState> state) {
	mInitialState = state;
	LOG() << "set initialState " << printState(state);
}

void Engine::transitionState(std::shared_ptr<IState> state) {
	mTransitState = state;
	LOG() << "set transitionState " << printState(state);
}

void Engine::start() {
	LOG() << "start engine " << printState(mInitialState);
	mHaltState->setEngine(this);
	StateInfo *sinfo = mStateMap[mInitialState];

	if (!mInitialState) {
		LOG() << "unset initialState";
		return;
	}
...
void Engine::fillStateStack(StateInfo *info, bool checkActive) {
	...
	do {
		...
		LOG() << "mStateStack push " << info->cState->toString();
...
void Engine::fillStateVector() {
	...
	while (!mStateStack.empty()) {
		...
		LOG() << "mStateVec push " << mStateStack.top()->cState->toString();
		LOG() << "mStateStack pop " << mStateStack.top()->cState->toString();
		mStateStack.pop();
	}
}
