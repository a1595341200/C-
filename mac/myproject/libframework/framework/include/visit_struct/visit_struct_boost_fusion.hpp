...
private:
	...
	struct fusion_visitor_pair {
		...
		explicit fusion_visitor_pair(V v, T1 t1, T2 t2)
			: visitor(std::forward<V>(v)), instance_1(std::forward<T1>(t1)), instance_2(std::forward<T2>(t2)) {}

		template<typename Index>
		VISIT_STRUCT_CXX14_CONSTEXPR void operator()(Index) const {
			accessor<Index::value> a;
			std::forward<V>(visitor)(field_name<Index::value>(),
									 a(std::forward<T1>(instance_1)),
									 a(std::forward<T2>(instance_2)));
		}
	};
...
public:
	...
	static void apply(V &&v, T1 &&t1, T2 &&t2) {
		using Indices = mpl::range_c<unsigned, 0, fusion::result_of::size<S>::value>;
		using fv_t = fusion_visitor_pair<decltype(std::forward<V>(v)),
