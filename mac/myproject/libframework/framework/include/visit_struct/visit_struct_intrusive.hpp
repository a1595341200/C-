...
struct structure_helper<TypeList<Ms...>> {
	...
	VISIT_STRUCT_CXX14_CONSTEXPR static void apply_visitor(V &&visitor, S &&structure_instance) {
		...
		int dummy[] =
			{(member_helper<Ms>::apply_visitor(std::forward<V>(visitor), std::forward<S>(structure_instance)), 0)...,
				0};
...
> {
	...
	-> visit_struct::type_c<typename detail::Find_t<typename T::Visit_Struct_Registered_Members_List__,
													idx>::value_type>;
